...
class ZmopClient {
    ...
    public function execute($request) {
        ...
        $sysParams[$this->signParam] = RSA::sign($apiParamsQuery, $this->privateKeyFilePath);


        //加密,只对业务参数进行加密
        $encryptedApiParams[$this->bizParam] = RSA::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKeyFilePath);

        //如果是文件上传，那么需要把文件参数也放到POST数组里面
        $files = $request->getFileParas();
        if($files != null){
            foreach ($files as $k => $v) {
                $encryptedApiParams [$k] = Http::getFilePath ($v);
            }
        }

        //系统参数放入GET请求串
        $requestUrl = $this->gatewayUrl . "?";
        $requestUrl .= Http::buildQueryWithEncode($sysParams);

        //发起HTTP请求
        try {
            $resp = Http::curl($requestUrl, $encryptedApiParams ,$this->charset);
        } catch (Exception $e) {
        ...
        if ("json" == $this->format) {
            ...
            if($encrypted){
                $biz_response = RSA::rsaDecrypt($biz_response, $this->privateKeyFilePath);
            }
        ...
        if(isset($respObject->biz_response_sign)){
            ...
            $verifyResult = RSA::verify($biz_response, $bizResponseSign, $this->zhiMaPublicKeyFilePath);
...
//            var_dump($bizRespObject->error_code);exit;
//            $logger = new LtLogger;
...
//        }
    ...
    public function generatePageRedirectInvokeUrl($request){
        ...
        $systemParams[$this->signParam] = RSA::sign($apiParamsQuery, $this->privateKeyFilePath);
        $systemQueryParams = Http::buildQueryWithEncode($systemParams);

        $url = $this->gatewayUrl;
        $url .= ('?' . $systemQueryParams);


        $bizParam =  RSA::rsaEncrypt($apiParamsQuery, $this->zhiMaPublicKeyFilePath);
    ...
    public function decryptAndVerifySign($encryptedResponse, $sign){
        $decryptedResponse = RSA::rsaDecrypt($encryptedResponse, $this->privateKeyFilePath);
        $success = RSA::verify($decryptedResponse, $sign, $this->zhiMaPublicKeyFilePath);
        if($success){
    ...
    public function generateSign($request){
        ...
        return RSA::sign($paramStr, $this->privateKeyFilePath);
    }
    ...
    public function generateEncryptedParam($request){
        $paramStr = $this->getBizParamStr($request);
        return RSA::rsaEncrypt($paramStr, $this->zhiMaPublicKeyFilePath);
    }
    ...
    private function getBizParamStr($request){
        ...
        $apiParamsQuery = Http::buildQueryWithEncode($apiParams);
        return $apiParamsQuery;
    }

    private function getSystemParams($request){
        if (Http::checkEmpty($this->charset)) {
            $this->charset = "UTF-8";
        }

        $iv=null;
        if(!Http::checkEmpty($request->getApiVersion())){
